<script lang="ts">
  ...
  async function deleteSelected() {
    ...
    if (selection.length >= thumbnails.length) {
      error = 'Cannot delete all pages. Keep at least one.';
      return;
    }
    busy = true;
    error = '';
    try {
      ...
      const resultHandle = await api.deletePages(handle, selection, onProgress);
  ...
  {:else if thumbnails.length > 0}
    <div class="flex items-center justify-between">
      <p class="text-sm text-slate-500">
        {thumbnails.length} pages — click to select pages to delete
        {#if selection.length > 0}
          <span class="ml-2 font-medium text-red-600">({selection.length} selected)</span>
        {/if}
      </p>
    ...
    <button
      ...
      class="w-full py-2.5 text-sm font-semibold rounded-lg bg-red-600 hover:bg-red-500 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
    >
      {busy ? 'Deleting…' : selection.length === 0 ? 'Select pages to delete' : `Delete ${selection.length} page${selection.length === 1 ? '' : 's'} & Download`}
    </button>
  {/if}
</div>
